...
goal_model_dir = os.path.join(os.path.split(os.path.realpath(__file__))[0], '..', '..', 'husky',
                              'husky_gazebo', 'models', 'Target', 'model.sdf')
...
class Env():
    def __init__(self, is_training):
        ...
        self.pub_cmd_vel = rospy.Publisher('/husky_velocity_controller/cmd_vel', Twist, queue_size=10)
    ...
    def getState(self, scan):
        ...
        arrive = False
        gap = int(len(scan.ranges) / 9) - 1

        for i in range(0, len(scan.ranges), gap):
