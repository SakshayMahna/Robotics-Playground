    ...
    if is_training:
        ...
        while True:
            ...
            while True:
                ...
                n_a = np.array([0, 0])
                n_a[0] = np.clip(np.random.normal(a[0], var), 0., 1.)
                n_a[1] = np.clip(np.random.normal(a[1], var), -0.5, 0.5)

                state_, r, done, arrive = env.step(n_a, past_action)
                time_step = agent.perceive(state, n_a, r, state_, done)
